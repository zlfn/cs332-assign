...
object Huffman {
  ...
  case class Leaf(char: Char, weight: Int) extends CodeTree {
    def contains(char: Char): Boolean = this.char == char
  }
  ...
  def times(chars: List[Char]): List[(Char, Int)] = 
    chars.groupBy(identity).map{case (c, list) => (c, list.size)}.toList
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] =
    freqs.sortBy(f => f._2).map{case (c, freq) => Leaf(c, freq)}
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
    case Nil => trees
    case some :: Nil => trees
    case some :: other :: anothers => {
      def recurInsert(e: CodeTree, list: List[CodeTree]): List[CodeTree] =
        list match {
          case Nil => List(e)
          case some :: others if weight(e) <= weight(some) => e :: list
          case some :: others => some :: recurInsert(e, others)
        }
      recurInsert(makeCodeTree(some, other), anothers)
    }
  }
   ...
   */
  def until(
    singletonFun: List[CodeTree] => Boolean = singleton, 
    combineFun: List[CodeTree] => List[CodeTree] = combine)
    (trees: List[CodeTree]): List[CodeTree] = 
    trees match {
      case _ if singleton(trees) => trees
      case _ => until(singletonFun, combineFun)(combine(trees))
    }
  ...
  def createCodeTree(chars: List[Char]): CodeTree =
    until()(makeOrderedLeafList(times(chars))).head
