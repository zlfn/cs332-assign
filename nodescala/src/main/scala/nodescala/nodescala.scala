      ...
      token => Future {
        async { 
          while(token.nonCancelled) {
            ...
            Future { respond(exc, token, handler(req)) }
          } 
        }
      }
    }
    return Subscription(listenerSubscription, subscription)
  }
}
...
object NodeScala {
  ...
  object Exchange {
    def apply(exchange: HttpExchange) = new Exchange {
      ...
      def request: Request = {
        val headers = for ((k: String, vs: List[String]) <- exchange.getRequestHeaders.asScala) yield (k, vs.toList) 
