...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.filter{case (block, _) => !explored.contains(block)}.toStream
  ...
  def from(initial: Stream[(Block, List[Move])],
           explored: Set[Block]): Stream[(Block, List[Move])] = initial match {
             ...
             case head #:: tail => 
               ...
               from(tail ++ newNeighborsOnly(neighborsWithHistory(head._1, head._2), explored),
                 explored + head._1)
           }
  ...
  lazy val pathsToGoal: Stream[(Block, List[Move])] = 
    pathsFromStart.flatMap {
      case (block, moveList) if done(block) => Stream((block, moveList))
      case _ => Stream.Empty
    }
